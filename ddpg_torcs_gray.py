...
class History(object):
    def __init__(self, history_length = 4, screen_height = 64, screen_width = 64):
        self.history = np.zeros([history_length, screen_height, screen_width], dtype=np.float32)
    def add(self, screen):
        self.history[:-1] = self.history[1:]
        self.history[-1] = screen / 255.0
    def fill(self, screen):
        for i in range(len(self.history)):
            self.history[i, :, :] = screen / 255.0
    def reset(self):
        self.history *= 0
    def get(self):
        return self.history.reshape(64, 64, 4)
...
def playGame(checkpoints=None, train_indicator=1, eps=1.0):    #1 means Train, 0 means simply Run
    ...
    LRA = 0.008    #Learning rate for Actor
    ...
    if vision:
        state_dim = (64, 64, 4)  #of sensors input
    else:
        state_dim = 29
    np.random.seed(1337)
    ...
    for i in range(episode_count):
        ...
        if vision:
            history.fill(cv2.cvtColor(ob.img, cv2.COLOR_RGB2GRAY))
            s_t = history.get()
        else:
        ...
        for j in range(max_steps):
            ...
            noise_t[0][1] = train_indicator * max(epsilon, 0) * OU.function(a_t_original[0][1],  1.0 , 1.00, 0.10)
            ...
            if vision:
                ...
                history.add((cv2.cvtColor(ob.img, cv2.COLOR_RGB2GRAY)))
            ...
            if vision:
                new_states_t = new_states.reshape((-1,) + state_dim)
                predict_t = actor.target_model.predict(new_states)
                target_q_values = critic.target_model.predict([new_states_t, predict_t])
            else:
...
if __name__ == "__main__":
    episode = int(sys.argv[1]) if len(sys.argv) >=2 else 0
