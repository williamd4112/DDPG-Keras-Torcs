...
def playGame(checkpoints=None, train_indicator=1, eps=1.0):    #1 means Train, 0 means simply Run
    ...
    for i in range(episode_count):
        ...
        print("TOTAL REWARD @ " + str(i) +"-th Episode  : Reward "+ str(total_reward) + "  EPS " + str(epsilon), file=log_file)
        print("Total Step: " + str(step) + ' Max: ' + str(max_reward) + ' Min: ' + str(min_reward), file=log_file)
        print("")
...
if __name__ == "__main__":
    episode = int(sys.argv[1]) if len(sys.argv) >=2 else 18
    eps = float(sys.argv[2]) if len(sys.argv) >= 3 else 0.97
